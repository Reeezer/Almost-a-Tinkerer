...
public class Conveyor extends Building
{
	...
	public void renderItems(SpriteBatch batch, int tileSize)
	{
		...
		for (Item item : items)
		{
			...
			// FIXME if we do want to correct corner animation, we can take off (- (tileSize
			// * xOrientation) / 2) but now the items goes completely on the next
			// tile
			int xPixPosition = ((x * tileSize)
					+ (int) (position * (float) tileSize * xOrientation) /*- (tileSize * xOrientation) / 2*/);
			int yPixPosition = ((y * tileSize)
					+ (int) (position * (float) tileSize * yOrientation) /*- (tileSize * yOrientation) / 2*/);
